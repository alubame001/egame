...
// NicknameCanRegistered checks if the username or nickname is available.
func NicknameCanRegistered(userName string, nickName string) (bool, bool, error) {
	cond := orm.NewCondition()
	cond = cond.Or("UserName", userName).Or("NickName", nickName)

	var maps []orm.Params
	o := orm.NewOrm()
	//如果nicekname 和username中有相同的，就不行。
	n, err := o.QueryTable("user").SetCond(cond).Values(&maps, "UserName", "NickName")
	if err != nil {
	...
	if n > 0 {
		for _, m := range maps {
			...
			if e2 && orm.ToStr(m["Nickname"]) == nickName {
				e2 = false
			}
		}
	}
...
// 每60秒只能修改一次nickname
func NicknameEnabledToChange(user *models.User) bool {
	//	beego.Trace("NicknameEnabledToChange:", user.LastNickNameUpdated)
	//	beego.Trace(beego.Date(user.LastNickNameUpdated, setting.DateFormat))
	//beego.Trace(setting.UpdateFrequencyLimitMinute)
	seconds := int(time.Since(user.LastNickNameUpdated).Seconds())
	beego.Trace("seconds:", seconds)
	if seconds > 60*setting.UpdateFrequencyLimitMinute {
		return true
	} else {
...
func EnableToSendActiveEmail(user *models.User) bool {
	beego.Trace("EnableToSendActiveEmail:", user.LastActiveEmailUpdated)
	beego.Trace(beego.Date(user.LastActiveEmailUpdated, setting.DateFormat))

	seconds := int(time.Since(user.LastActiveEmailUpdated).Seconds())
	beego.Trace("seconds:", seconds)
	if seconds > 60*setting.ActiveCodeLives {
		return true
	} else {
		beego.Trace("EnableToSendActiveEmail:  denied")
		return false
	}
	//acitve_code_live_minutes
}
...
func InsertKey(user *models.User) error {
	beego.Info("InsertKey")
	var key models.Key
	key.User = user
	key.Spwd = utils.EncodePassword(utils.ToStr(time.Now()), user.Rands)
	beego.Info("Spwd", key.Spwd)
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	key.Spwd = utils.SubString(key.Spwd, 0, r.Intn(20)+20)
	beego.Info("New Spwd", key.Spwd)
	key.Ckey = user.NickName
	key.Shash = utils.EncodeSha256(key.Spwd)
	key.Nonce = 1
	//beego.Info("Shash", key.Shash)
	return key.Insert()
}

func InsertDeposit(user *models.User) error {
	//

	var deposit models.Deposit
	deposit.User = user
	deposit.Balance = 1000000
	...
	/*
		...
		deposit.Balance = 1000000
		o := orm.NewOrm()
		if id, err := o.Insert(&deposit); err != nil {
			return err
		} else {

			if res, err := o.Raw("update user set deposit_id = ?  WHERE id = ?", id, user.Id).Exec(); err != nil {

				return err
			} else {
				num, err := res.RowsAffected()
				fmt.Println("user table row affected nums: ", num)
				if num == 0 {
					beego.Info("err:", err)
...
func LoginUser(user *models.User, ctx *context.Context, remember bool) {
	...
	beego.Trace("LoginUser:", user)
	beego.Trace("IP:", ctx.Input.IP())
	...
	//fmt.Print("auth_user_id:", user.Id)
	if remember {
...
func GetUserIdFromSession(sess session.SessionStore) int {
	if id, ok := sess.Get("auth_user_id").(int); ok && id > 0 {
		beego.Info("GetUserIdFromSession Id:", id)
		return id
	}
	return 0
}
...
func GetUserFromSession(user *models.User, sess session.SessionStore) bool {
	//	beego.Trace(" GetUserFromSession user sess:", sess)
	id := GetUserIdFromSession(sess)
	if id > 0 {
		u := models.User{Id: id}
		//beego.Trace(" GetUserFromSession user id :", u)
		if u.Read() == nil {
			*user = u
			return true
		}
...
func VerifyUserActiveCode(user *models.User, code string) bool {
	...
	beego.Trace("VerifyUserActiveCode start")
	...
	beego.Trace("VerifyUserActiveCode failure")
	return false
}
...
//  ip record
func InsertLoginIp(user *models.User, address string) error {
	// use random salt encode password
	//fmt.Print("InsertLoginIp user:", user)
	//fmt.Print("InsertLoginIp Address:", address)
	if setting.EnableIpRecord {
		var ip models.Ip
		ip.Address = strings.ToLower(address)
		ip.User = user
...
func UpdateUserIp(user *models.User, address string) error {
	if setting.EnableIpRecord {
		user.LastLoginAddress = strings.ToLower(address)
...
func UpdateUserLastActiveEmailUpdated(user *models.User) error {
	//if setting.EnableIpRecord {
	if true {

		user.LastActiveEmailUpdated = time.Now()
		return user.Update()
	} else {
		return nil
	}
