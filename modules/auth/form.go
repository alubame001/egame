...
/*只有能修改的栏位才加上，不能加上username等不可修改的栏位*/
...
func (form *ProfileForm) Valid(v *validation.Validation) {
	...
	//fmt.Print(form.NickName)
	/*
		if form.UserName == form.NickName {
			v.SetError("NickName", "auth.nickname_equal_username")

		} else {
			e1, e2, _ := NicknameCanRegistered("hello", form.NickName)
