...
type User struct {
	Id                     int
	Bets                   []*Bet           `orm:"reverse(many)"`                       // 一对多
	Keys                   []*Key           `orm:"reverse(many)"`                       //一对多
	Deposit                *Deposit         `orm:"null;rel(one);on_delete(do_nothing)"` //一对一
	...
	IsActiveEmail          bool             `orm:"index"` //EMAIL认证通过
	IsActiveMobile         bool             `orm:"index"` //手机认证通过
	IsActiveGa             bool             `orm:"index"` //ga认证通过
	IsForbid               bool             `orm:"index"`
	Lang                   int              `orm:"index"`
	LangAdds               SliceStringField `orm:"size(50)"`
	Rands                  string           `orm:"size(10)"`
	Progress               int              `orm:"default(0)"`
	LastLoginAddress       string           `orm:"size(20)"` //Last Login IP
	LastLoginTime          time.Time        `orm:"auto_now"`
	Created                time.Time        `orm:"auto_now_add"`
	Updated                time.Time        `orm:"auto_now"`
	LastNickNameUpdated    time.Time        `orm:"null"` //Nicakname最後更新时间
	LastActiveEmailUpdated time.Time        `orm:"null"` //激活email最後发送时间
	//LastPostUpdated  time.Time        `` //最後新帖时间
	//LastCommentCreated time.Time        `orm:"auto_now_add"` //最後回覆时间
}
