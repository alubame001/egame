...
func (this *LoginRouter) Login() {
	...
	fmt.Printf("login key:", key)
	//fmt.Printf("ip table:", &ip)
	...
	if times, ok := utils.TimesReachedTest(key, setting.LoginMaxRetries); ok {
		...
		auth.InsertLoginIp(&user, this.Ctx.Input.IP())
		auth.UpdateUserIp(&user, this.Ctx.Input.IP())
...
func (this *RegisterRouter) Register() {
	...
	if err := auth.RegisterUser(user, form.UserName, form.Email, form.Password); err == nil {
		auth.InsertKey(user)
		auth.InsertDeposit(user)
		//beego.Info("insertDeposit:", i)
		auth.InsertLoginIp(user, this.Ctx.Input.IP())
		auth.UpdateUserIp(user, this.Ctx.Input.IP())
...
func (this *RegisterRouter) Active() {
	...
	beego.Trace("active code:", code)
	var user models.User

	if auth.VerifyUserActiveCode(&user, code) {
		beego.Trace(" auth.VerifyUserActiveCode sucess")
		user.IsActive = true
		user.IsActiveEmail = true /*added by Ray*/
		user.Rands = models.GetUserSalt()
		if err := user.Update("IsActiveEmail", "IsActive", "Rands", "Updated"); err != nil {
		...
		beego.Trace(" auth.VerifyUserActiveCode failure")
