...
func newMarketPrice(ep EventType, user, msg string) Martketprice {

	return Martketprice{0, "", 0, 0, ""}
}
	...
	for {
		...
		case sub := <-subscribe:
			if !isUserExist(subscribers, sub.Name) {
				...
				publish <- newEvent(EVENT_JOIN, sub.Name, "join")
