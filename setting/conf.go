...
var (
	...
	// mail when login  added by Ray
	EmailWhenLoginSucess bool
	EnableIpRecord       bool
)
...
func LoadConfig() *goconfig.ConfigFile {
	...
	//beego.Xsrfkey = 79dfgad4l54ghAdfGad5464Fdfa
	beego.EnableXSRF = true
	// xsrf token expire time
	beego.XSRFExpire = 86400 * 365

	driverName := Cfg.MustValue("orm", "driver_name", "postgres")
	dataSource := Cfg.MustValue("orm", "data_source", "postgres://root:h5654321@localhost:5432/cfd?sslmode=disable")
...
func reloadConfig() {
	...
	ActiveCodeLives = Cfg.MustInt("app", "acitve_code_live_minutes", 10)
	ResetPwdCodeLives = Cfg.MustInt("app", "resetpwd_code_live_minutes", 10)
	...
	MailFrom = Cfg.MustValue("mailer", "mail_from", "test")

	// set mailer connect args
	MailHost = Cfg.MustValue("mailer", "mail_host", "smtp.mailgun.org:25")
	MailAuthUser = Cfg.MustValue("mailer", "mail_user", "justtest@sandbox16a6a2aa4410439fa0516008821b4a4d.mailgun.org")
	MailAuthPass = Cfg.MustValue("mailer", "mail_pass", "justtest")
	...
	/* added by Ray */
	EmailWhenLoginSucess = Cfg.MustBool("app", "email_when_login_sucess")
	EnableIpRecord = Cfg.MustBool("app", "enable_ip_record")
...
func settingCompress() {
	...
	beego.AddFuncMap("compress_game", setting.Js.CompressJs)
