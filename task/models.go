// 参考 http://beego.me/docs/module/toolbox.md
package task
...
func InitTask() {
	...
	*/
	...
	var check_time = setting.CheckTime
	beego.Trace("check_time", check_time)
	//var needCheck = true
	/*
		if check_time != "0" {

			updateTask := toolbox.NewTask("check file update!", "0/"+check_time+" * * * * *", checkFileUpdates)
			if needCheckUpdate() {
	...
	*/
	//var scan_time = beego.AppConfig.String("yahoo_finance_scan_time")
	//yahooFinanceUpdateTask := toolbox.NewTask("check file update!", "0/"+scan_time+" * * * * *", yahoo)
	//var scan_time = beego.AppConfig.String("yahoo_finance_scan_time")
	/*
	FinanceUpdateTask := toolbox.NewTask("check file update!", "0/"+setting.CheckTime+" * * * * *", sina)
	toolbox.AddTask("Yahoo Finance Update Task", FinanceUpdateTask)
	*/

	LolUpdateTask := toolbox.NewTask("check file update!", "0/"+setting.CheckTime+" * * * * *", lolHttpGet)
	toolbox.AddTask("lolHttpGet  Update Task", LolUpdateTask)
	toolbox.StartTask()
